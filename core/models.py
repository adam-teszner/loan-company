...
class Product(models.Model):
    ...
    global_interest_rate = models.FloatField('Central banks\n interest rate in %', validators=[MinValueValidator(0), 
                                    MaxValueValidator(30)], blank=False, default=5.25)
    lombard_rate = models.FloatField('Central banks lombard\n interest rate in %', validators=[MinValueValidator(0), 
                                    MaxValueValidator(30)], blank=False, default=5.75)
    product_name = models.CharField(choices = products, max_length=2, default='L1')
    amount_requested = models.IntegerField(validators=[MinValueValidator(2000), 
                                    MaxValueValidator(50000)], default=10000, blank=False)
    loan_period = models.IntegerField(choices=list(zip(range(6, 49), range(6, 49))), blank=False, default=24)
